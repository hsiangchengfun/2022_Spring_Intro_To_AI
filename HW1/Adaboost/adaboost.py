...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        for j in range(len(features)):
          ...
          tmperror = 0
          feature_classifier = WeakClassifier(features[j])
          
          for i in range(len(weights)):
            value = feature_classifier.classify(iis[i])
            tmperror += (weights[i]* abs(value - labels[i]))
            
            #i modified the mean absolute error 
            # to the mean squre method  
          #   tmperror += (weights[i]* pow((value - labels[i]),2))
          # tmperror = pow(tmperror,0.5)            
                       
            
          if(tmperror < bestError):
            bestError = tmperror
            bestClf = feature_classifier
